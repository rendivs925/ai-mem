...
const DEFAULT_CHROMA_DATA_DIR = path.join(os.homedir(), '.ai-mem', 'chroma');
...
export class ChromaMcpManager {
   ...
   * the system keychain. Caches the result for 24 hours at ~/.ai-mem/combined_certs.pem.
   *
  ...
  private getCombinedCertPath(): string | undefined {
    const combinedCertPath = path.join(os.homedir(), '.ai-mem', 'combined_certs.pem');
